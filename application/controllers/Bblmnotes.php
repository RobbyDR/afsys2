...
class bblmnotes extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['get'] = $this->sql->select_table('tbl_bblmnotes', null, 'id DESC')->result_array();
        // function select_table_join($table, $column, $join_table, $join_on, $join, $where = array(1 => 1), $order_by = null)
        // $data['get'] = $this->sql->select_table_join('tbl_bblmnotes', 'tbl_bblmnotes.*,tbl_s2matkul.nama as namamatkul', 'tbl_s2matkul', 'tbl_s2matkul.id=tbl_bblmnotes.matkulid', 'left', array(1 => 1), 'tbl_bblmnotes.status ASC, tbl_bblmnotes.deadline ASC')->result_array();
        // $data['getmatkulid'] = $this->sql->select_table('tbl_s2matkul', ['tbl_s2matkul.status' => '1'], 'id ASC')->result_array();

        $data['subview'] = "bblmnotes/index";
    ...
    {
        if ($this->input->post()) {
            $id = $this->sql->insert_table('tbl_bblmnotes', [
    ...
    {
        if ($this->input->post()) {
            $data['get'] = $this->sql->select_table('tbl_bblmnotes', ['id' => $this->input->post('id')])->row_array();
            $this->load->view('bblmnotes/getview', $data);
        } else {
            echo "elor";
        }
    }
    ...
    public function getedit()
    {
        if ($this->input->post()) {
            $data['get'] = $this->sql->select_table('tbl_bblmnotes', ['id' => $this->input->post('id')])->row_array();
            // $data['getmatkulid'] = $this->sql->select_table('tbl_s2matkul', ['tbl_s2matkul.status' => '1'], 'id ASC')->result_array();
            $this->load->view('bblmnotes/getedit', $data);
        } else {
            echo "elor";
        }
    }
    ...
    {
        if ($this->input->post()) {
            $id = $this->sql->update_table('tbl_bblmnotes', [
                'nama' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('nama'))),
                'tgl1' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('tgl1'))),
                'deskripsi' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('deskripsi'))),
                'keterangan' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('keterangan'))),
                'status' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('status')))
            ], ['tbl_bblmnotes.id' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('id')))]);
    ...
    {
        $this->load->view('bblmnotes/smartparking');
    }
}
