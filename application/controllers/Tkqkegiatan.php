...
class Tkqkegiatan extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['get'] = $this->sql->select_table('tbl_tkqkegiatan', null, 'id DESC')->result_array();
        // function select_table_join($table, $column, $join_table, $join_on, $join, $where = array(1 => 1), $order_by = null)
        // $data['get'] = $this->sql->select_table_join('tbl_tkqkegiatan', 'tbl_tkqkegiatan.*,tbl_s2matkul.nama as namamatkul', 'tbl_s2matkul', 'tbl_s2matkul.id=tbl_tkqkegiatan.matkulid', 'left', array(1 => 1), 'tbl_tkqkegiatan.status ASC, tbl_tkqkegiatan.deadline ASC')->result_array();
        // $data['getmatkulid'] = $this->sql->select_table('tbl_s2matkul', ['tbl_s2matkul.status' => '1'], 'id ASC')->result_array();

        $data['subview'] = "tkqkegiatan/index";
    ...
    {
        if ($this->input->post()) {
            $id = $this->sql->insert_table('tbl_tkqkegiatan', [
    ...
    {
        if ($this->input->post()) {
            $data['get'] = $this->sql->select_table('tbl_tkqkegiatan', ['id' => $this->input->post('id')])->row_array();
            $this->load->view('tkqkegiatan/getview', $data);
        } else {
            echo "elor";
        }
    }
    ...
    public function getedit()
    {
        if ($this->input->post()) {
            $data['get'] = $this->sql->select_table('tbl_tkqkegiatan', ['id' => $this->input->post('id')])->row_array();
            // $data['getmatkulid'] = $this->sql->select_table('tbl_s2matkul', ['tbl_s2matkul.status' => '1'], 'id ASC')->result_array();
            $this->load->view('tkqkegiatan/getedit', $data);
        } else {
            echo "elor";
        }
    }
    ...
    {
        if ($this->input->post()) {
            $id = $this->sql->update_table('tbl_tkqkegiatan', [
                'nama' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('nama'))),
                'tgl1' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('tgl1'))),
                'deskripsi' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('deskripsi'))),
                'keterangan' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('keterangan'))),
                'status' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('status')))
            ], ['tbl_tkqkegiatan.id' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('id')))]);
    ...
    {
        $data['judul'] = "Album Kegiatan TKQ";
        $data['getmenu'] = $this->sql->select_table('tbl_devmenuaf', ['tbl_devmenuaf.status' => '1', 'tbl_devmenuaf.jenis' => 'menu'], 'urutan ASC')->result_array();
        $data['getsubmenu'] = $this->sql->select_table('tbl_devmenuaf', ['tbl_devmenuaf.status' => '1', 'tbl_devmenuaf.jenis' => 'submenu'], 'urutan ASC')->result_array();

        $data['getutama'] = $this->sql->select_table('tbl_tkqkegiatan', ['id' => $id, 'status' => '1'])->row_array();

        $data['get'] = $this->sql->select_table('mst_files', [
            'afiliasi' => 'tkqkegiatan',
            'afiliasiid' => $id
        ])->result_array();

        $data['subview'] = "tkqkegiatan/album";
        $this->load->view('partial', $data);
    }
    ...
    public function tambahalbum()
    {
        if ($this->input->post()) {
            ...
            redirect("tkqkegiatan/album/" . $afiliasiid);
        } else {
            $this->session->set_flashdata('msg', 'Error!');
            $this->session->set_flashdata('msg_type', 'error');
            redirect("tkqkegiatan");
    ...
    public function geteditalbum()
    {
        if ($this->input->post()) {
            ...
            $this->load->view('tkqkegiatan/geteditalbum', $data);
        } else {
            echo "elor";
        }
    }
    ...
    public function editalbum()
    {
        if ($this->input->post()) {
            ...
            $this->session->set_flashdata('msg', 'Berhasil mengubah data!');
            $this->session->set_flashdata('msg_type', 'success');
            redirect("tkqkegiatan/album/" . $this->input->post('utamaid'));
        } else {
            $this->session->set_flashdata('msg', 'Error!');
            $this->session->set_flashdata('msg_type', 'error');
            redirect("tkqkegiatan");
