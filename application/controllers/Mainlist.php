...
class Mainlist extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['get'] = $this->sql->select_table('tbl_mainlist', null, 'id DESC')->result_array();
        // function select_table_join($table, $column, $join_table, $join_on, $join, $where = array(1 => 1), $order_by = null)
        // $data['get'] = $this->sql->select_table_join('tbl_mainlist', 'tbl_mainlist.*,tbl_s2matkul.nama as namamatkul', 'tbl_s2matkul', 'tbl_s2matkul.id=tbl_mainlist.matkulid', 'left', array(1 => 1), 'tbl_mainlist.status ASC, tbl_mainlist.deadline ASC')->result_array();
        // $data['getmatkulid'] = $this->sql->select_table('tbl_s2matkul', ['tbl_s2matkul.status' => '1'], 'id ASC')->result_array();

        $data['progress'] = $this->sql->select_table('tbl_mainlist', ['tbl_mainlist.status' => '2'])->num_rows();
        $data['progresstot'] = $this->sql->select_table('tbl_mainlist')->num_rows();

        $data['subview'] = "mainlist/index";
    ...
    {
        if ($this->input->post()) {
            $id = $this->sql->insert_table('tbl_mainlist', [
    ...
    {
        if ($this->input->post()) {
            $data['get'] = $this->sql->select_table('tbl_mainlist', ['id' => $this->input->post('id')])->row_array();
            $this->load->view('mainlist/getview', $data);
        } else {
            echo "elor";
        }
    }
    ...
    public function getedit()
    {
        if ($this->input->post()) {
            $data['get'] = $this->sql->select_table('tbl_mainlist', ['id' => $this->input->post('id')])->row_array();
            // $data['getmatkulid'] = $this->sql->select_table('tbl_s2matkul', ['tbl_s2matkul.status' => '1'], 'id ASC')->result_array();
            $this->load->view('mainlist/getedit', $data);
        } else {
            echo "elor";
        }
    }
    ...
    {
        if ($this->input->post()) {
            $id = $this->sql->update_table('tbl_mainlist', [
                'nama' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('nama'))),
                'tgl1' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('tgl1'))),
                'tgl2' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('tgl2'))),
                'deskripsi' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('deskripsi'))),
            ...
            ], ['tbl_mainlist.id' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('id')))]);
