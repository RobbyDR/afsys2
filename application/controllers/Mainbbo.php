...
class Mainbbo extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['get'] = $this->sql->select_table('tbl_mainbbo', null, 'id DESC')->result_array();
        // function select_table_join($table, $column, $join_table, $join_on, $join, $where = array(1 => 1), $order_by = null)
        // $data['get'] = $this->sql->select_table_join('tbl_mainbbo', 'tbl_mainbbo.*,tbl_s2matkul.nama as namamatkul', 'tbl_s2matkul', 'tbl_s2matkul.id=tbl_mainbbo.matkulid', 'left', array(1 => 1), 'tbl_mainbbo.status ASC, tbl_mainbbo.deadline ASC')->result_array();
        // $data['getmatkulid'] = $this->sql->select_table('tbl_s2matkul', ['tbl_s2matkul.status' => '1'], 'id ASC')->result_array();

        $data['subview'] = "mainbbo/index";
    ...
    {
        if ($this->input->post()) {
            $id = $this->sql->insert_table('tbl_mainbbo', [
    ...
    {
        if ($this->input->post()) {
            $data['get'] = $this->sql->select_table('tbl_mainbbo', ['id' => $this->input->post('id')])->row_array();
            $this->load->view('mainbbo/getview', $data);
        } else {
            echo "elor";
        }
    }
    ...
    public function getedit()
    {
        if ($this->input->post()) {
            $data['get'] = $this->sql->select_table('tbl_mainbbo', ['id' => $this->input->post('id')])->row_array();
            // $data['getmatkulid'] = $this->sql->select_table('tbl_s2matkul', ['tbl_s2matkul.status' => '1'], 'id ASC')->result_array();
            $this->load->view('mainbbo/getedit', $data);
        } else {
            echo "elor";
        }
    }
    ...
    {
        if ($this->input->post()) {
            $id = $this->sql->update_table('tbl_mainbbo', [
                'nama' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('nama'))),
                'tgl1' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('tgl1'))),
                'deskripsi' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('deskripsi'))),
                'keterangan' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('keterangan'))),
                'status' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('status')))
            ], ['tbl_mainbbo.id' => $this->security->xss_clean($this->sql->sanitasi($this->input->post('id')))]);
