<div class="container-fluid scrollarea">
    <div class="row ">
        ...
        <div class="table-responsive small">
            <table class="table table-striped table-sm" id="bblmnotestable">
                <thead>
                    <tr>
                        <th scope="col">#</th>
                ...
                <tbody>
                    <?php foreach ($get as $row): ?>
                        ...
                        <tr class="<?= $row['status'] == "2" ? 'fw-lighter' : ''; ?>">
                            <td><?= $row['id'] ?></td>
                            <td><?= $row['nama'] ?></td>
                            <td class="text-break"><?= $row['deskripsi'] ?></td>
                            <td><?= $row['tgl1'] ?></td>
...
<div class="modal modal-xl fade" id="tambahModal" tabindex="-1" aria-labelledby="tambahModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="<?= base_url() ?>bblmnotes/tambah" method="POST" enctype="multipart/form-data">
...
<script>
    $(document).ready(function() {
        $("#viewModal").on('shown.bs.modal', function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "<?= base_url() ?>bblmnotes/getview",
                data: {
                    id: id
                }
        ...
        $("#editModal").on('shown.bs.modal', function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "<?= base_url() ?>bblmnotes/getedit",
                data: {
                    id: id
                }
            }).done(function(response) {
